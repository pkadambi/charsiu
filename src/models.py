...
class Wav2Vec2ForFrameClassificationSAT(Wav2Vec2ForCTC):
    def __init__(self, config, ivector_size=64):
        super().__init__(config)
        ...
        self.lm_head_ivec = nn.Linear(output_hidden_size + ivector_size, config.vocab_size )
...
class Wav2Vec2ForCTCAndPretraining(Wav2Vec2ForPreTraining):
    ...
    def __init__(self, config):
        ...
        self.cnn = ConvBank(input_dim=config.vocab_size - 1,
                            output_class_num=config.hidden_size,
                            kernels=[1],
                            cnn_size=config.hidden_size,
