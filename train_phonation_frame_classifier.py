...
parser.add_argument('--gtdf_path', default='./phonation_data.csv')
parser.add_argument('--audio_dir', default='/home/prad/datasets/phonation_data/')
parser.add_argument('--manual_textgrids_dir', default='/home/prad/datasets/phonation_data/extracted_manual_textgrids')
parser.add_argument('--model_output_dir', default='./phonation_loso_model')
...
parser.add_argument('--results_dir', default='./results_trained_frame')
...
parser.add_argument('--dataset_dir', default='./data/phonation_dataset_framewise_nopause')
...
gtdf = pd.read_csv(args.gtdf_path)
gtdf = gtdf[gtdf['RatePhonationStepPauseCount']==0]
gtdf = gtdf[~pd.isna(gtdf['start_time_groundtruth'])]
audio_files =[os.path.join(audio_dir, stepid)+'.wav' for stepid in list(gtdf['sessionStepId'].values)]
# audio_files =

unmatched_manual_textgrid_files = [os.path.join(manual_textgrids_dir, audfile.split('/')[-1].split('.')[0] + '.TextGrid')
                                   for audfile in audio_files]
# audio_files = [os.path.join(audio_dir, _path.split('/')[-1].split('.')[0]+ '.wav')
#                for _path in unmatched_manual_textgrid_files]

# audio_files = [os.path.join(audio_dir, tgname.split('/')[-1][:-8]+'wav') for tgname in estimated_tgs]
''' Load Models '''
...
def speakerwise_train_test_split(dataset, speaker_id, speaker_col='speaker_id', age_restrict=False, age_range=False):
    test_split = dataset.filter(lambda example: speaker_id==example[speaker_col])
    if age_restrict:
        ...
        train_split = dataset.filter(lambda example: speaker_id!=example[speaker_col])
...
if not os.path.exists(dataset_dir):
    ...
    tg_dir = os.path.join(audio_dir, 'extracted_manual_textgrids')
    psd = PhonationDataset(audio_paths=audio_files, lables_df=gtdf, textgrids_dir=tg_dir)
    phonation_dataset = psd.return_as_datsets()
    phonation_dataset = phonation_dataset.map(prepare_framewise_dataset)
    phonation_dataset.save_to_disk(dataset_dir)
    unique_speakers = list(set(list(phonation_dataset['speaker_id'])))
    ...
    phonation_dataset = load_from_disk(dataset_dir)
    unique_speakers = list(set(list(phonation_dataset['speaker_id'])))
...
def write_textgrid_alignments_for_speaker(charsiu_aligner, loso_dataset, output_dir):
    ...
    for ii, audiofilepath in tqdm.tqdm(enumerate(audiofiles)):
        file_id = loso_dataset['sessionid'][ii]
...
if __name__ == "__main__":
    ...
    phonation_dataset = phonation_dataset.filter(lambda example: file_to_remove not in example)
    unique_speakers = list(set(list(phonation_dataset['speaker_id'])))
    ...
    for jj, loso_speaker_id in enumerate(unique_speakers):
        ...
        loso_manual_textgrids = [tgpath for tgpath in unmatched_manual_textgrid_files
                                 if loso_speaker_id==PhonationDataset.speaker_id_from_tgpath(tgpath)]
        loso_estimated_textgrids = [os.path.join(results_dir, loso_speaker_id, tgpath.split('/')[-1])
                                    for tgpath in unmatched_manual_textgrid_files if loso_speaker_id==PhonationDataset.speaker_id_from_tgpath(tgpath)]
        ...
        del charsiu
        #TODO: replace with a phoneme bounds error function
        pt_acc, pt_numcorrect, pt_numpredicted = calc_accuracy_between_textgrid_lists(loso_manual_textgrids, loso_estimated_textgrids, manual_phonekey='phones', aligner_phonekey='phones')
