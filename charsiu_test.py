...
transcripts = {}
for filename in manual_textgrid_files:
    fname = filename.split('/')[-1]
    speaker_dir = filename.split('/')[-2]
    # print(fname)
    fname = os.path.join(audio_dir, speaker_dir, fname[:-8]+'lab')
    # print(fname)
    # break
    f = open(fname)
    _transcript = f.read()
...
charsiu = charsiu_forced_aligner('charsiu/en_w2v2_fs_10ms')
charisu_tg_files = []
error_files = []
for tgfilepath in tqdm.tqdm(manual_textgrid_files):
    audiofname = tgfilepath.split('/')[-1][:-8] + 'wav'
    speaker_dir = tgfilepath.split('/')[-2]
    tgfilename = tgfilepath.split('/')[-1]
    audio_path = os.path.join(audio_dir, speaker_dir, audiofname) + '.wav'

    output_tg_dir = os.path.join('./charsiu_w2v2_attention_aligner_20ms/', speaker_dir)
    ...
    _transcript = transcripts[tgfilepath]
    # print(tg_filepath)
    try:
