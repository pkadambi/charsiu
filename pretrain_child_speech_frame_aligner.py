...
from transformers import Wav2Vec2CTCTokenizer, Wav2Vec2ForPreTraining
...
parser.add_argument('--results_dir', default='./results_pretraining')
...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]], device='cpu') -> Dict[
        str, torch.Tensor]:
        ...
        masked_seq_lengths = self.model._get_feat_extract_output_lengths(batch['input_values'].shape[-1])
        masked_seq_lengths = int(masked_seq_lengths)

        # if batch.get('attention_mask') is not None:
        #     batch['sub_attention_mask'] = self.model._get_feature_vector_attention_mask(masked_seq_lengths,
        #                                                                                 batch['attention_mask'])

        batch_size, raw_sequence_length = batch['input_values'].shape
        features_shape = (batch_size, masked_seq_lengths)

        masked_time_indices = _compute_mask_indices(features_shape, mask_prob=0.1, mask_length=2,
                                                    attention_mask=batch.get('sub_attention_mask'))

        sampled_negative_indices = _sample_negative_indices(features_shape, self.model.config.num_negatives,
                                                            mask_time_indices=masked_time_indices)

        # batch['mask_time_indices'] = torch.Tensor(masked_time_indices).type(torch.long).to(device)
        batch['mask_time_indices'] = torch.tensor(masked_time_indices, dtype=torch.long, device=device)
...
if __name__ == "__main__":
    ...
    for jj, loso_speaker_id in enumerate(unique_speakers):
        ...
        ptmdl = Wav2Vec2ForPreTraining.from_pretrained('facebook/wav2vec2-base')
        missing_keys = ['project_hid.weight', 'project_q.weight', 'quantizer.weight_proj.bias', 'quantizer.codevectors', 'quantizer.weight_proj.weight', 'project_hid.bias', 'project_q.bias']
        fb_ptmdl_dict = ptmdl.state_dict()
        transfer_dict = {key : value for (key,value) in fb_ptmdl_dict.items() if key in missing_keys}

        charsiu_ptmdl = Wav2Vec2ForPreTraining.from_pretrained('charsiu/en_w2v2_fc_10ms')
        charsiu_state_dict = {}
        charsiu_state_dict.update(transfer_dict)
        charsiu_ptmdl.load_state_dict(charsiu_state_dict, strict=False)
        data_collator = DataCollatorWithPadding(model=charsiu_ptmdl, processor=processor, padding=True)
        ...
        charsiu.aligner = charsiu_ptmdl #update the aligner with the trained model
