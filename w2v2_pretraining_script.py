...
def get_child_speech_dataset(loso_speaker_id, model_name, dataset_dir='./data/child_speech_framewise',
    ...
    def speakerwise_train_test_split(dataset, speaker_id, speaker_col='speaker_id', age_restrict=False,
                                     age_range=False):
        ...
        if age_restrict:
            ...
            train_split = dataset.filter(
                lambda example: all([incl_age in example[speaker_col][:2] for incl_age in include_ages]) and
                                speaker_id not in example[speaker_col])
        else:
    ...
    train_dataset, loso_dataset = get_loso_dataset(leave_out_speaker_id=loso_speaker_id,
                                                   ...
                                                   age_range=False,
                                                   age_restrict=False)
    raw_datasets["validation"] = loso_dataset
...
def main():
    ...
    '''
    raw_datasets = get_child_speech_dataset(loso_speaker_id='0511_F_IG', model_name=args.model_name_or_path)
